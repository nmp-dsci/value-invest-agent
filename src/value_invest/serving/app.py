...
HEADLINE_ITEMS = {
    ...
    "balance": ["Total Assets", "Total Debt", "Cash And Cash Equivalents", "Stockholders Equity", "Net Debt", "Working Capital"],
    "cashflow": ["Operating Cash Flow", "Capital Expenditure", "Free Cash Flow", "Cash Dividends Paid", "Repurchase Of Capital Stock"],
}
...
def create_app() -> FastAPI:
    ...
    def health() -> dict:
        c = con()
        return {"ok": True, "mode": "demo" if s.demo_mode else "dev", "tables": db.table_counts(c), "milestone": "M1"}

    @app.get("/api/funnel")
    def funnel() -> dict:
        c = con()
        listed = c.execute("SELECT count(*) FROM vi.videos").fetchone()[0]
        in_window = c.execute("SELECT count(*) FROM vi.videos WHERE year_bucket IS NOT NULL").fetchone()[0]
        kinds = _rows(c, "SELECT coalesce(kind,'unlabelled') AS kind, count(*) AS n FROM vi.videos WHERE year_bucket IS NOT NULL GROUP BY 1 ORDER BY 2 DESC")
        per_year = _rows(c, """SELECT year_bucket, count(*) AS videos,
        ...
        indexed = c.execute("SELECT count(*) FROM vi.videos WHERE in_sample AND transcript_status='indexed'").fetchone()[0]
        return {"listed": listed, "in_window": in_window, "kinds": kinds, "per_year": per_year, "sampled": sampled, "indexed": indexed,
                "since": str(s.since), "until": str(s.until), "per_year_target": s.sample_per_year, "seed": s.sample_seed}

    @app.get("/api/videos")
    def videos(sample: bool = True, kind: str | None = None, year: str | None = None, limit: int = 2000) -> list[dict]:
        c = con()
        where, params = [], []
        if sample:
            where.append("v.in_sample")
        if kind:
            where.append("v.kind = ?"); params.append(kind)
        if year:
            where.append("v.year_bucket = ?"); params.append(year)
        w = ("WHERE " + " AND ".join(where)) if where else ""
        rows = _rows(c, f"""SELECT v.video_id, v.title, v.published_at, v.duration_s, v.view_count, v.kind, v.primary_ticker,
                            ...
                            {w} ORDER BY v.published_at DESC LIMIT {int(limit)}""", params)
        for r in rows:
    ...
    def video(video_id: str, before_days: int = 365, after_days: int = 730) -> dict:
        c = con()
        v = _rows(c, """SELECT v.*, l.confidence, l.rationale, l.tickers, t.name AS company, t.currency, t.exchange, t.benchmark
                        FROM vi.videos v LEFT JOIN vi.title_labels l USING (video_id)
                        LEFT JOIN vi.tickers t ON t.ticker = v.primary_ticker WHERE v.video_id = ?""", [video_id])
        if not v:
        ...
        out: dict[str, Any] = {"video": row, "t0": t0, "prices": [], "benchmark": [], "statements": {}, "statement_periods": [], "transcript": None, "chunks": [], "coverage": None}
        if ticker and t0:
            lo, hi = t0 - timedelta(days=before_days), t0 + timedelta(days=after_days)
            out["prices"] = _rows(c, "SELECT date, close, adj_close FROM vi.prices WHERE ticker = ? AND date BETWEEN ? AND ? ORDER BY date", [ticker, lo, hi])
            if row.get("benchmark"):
                out["benchmark"] = _rows(c, "SELECT date, adj_close FROM vi.prices WHERE ticker = ? AND date BETWEEN ? AND ? ORDER BY date", [row["benchmark"], lo, hi])
            at = _rows(c, "SELECT date, close FROM vi.prices WHERE ticker = ? AND date <= ? ORDER BY date DESC LIMIT 1", [ticker, t0])
            ...
            for m in s.horizons_months:
                ...
                r = _rows(c, "SELECT date, close FROM vi.prices WHERE ticker = ? AND date >= ? ORDER BY date LIMIT 1", [ticker, d])
            ...
            out["statements"] = {k: {"headline": [i for i in HEADLINE_ITEMS[k] if i in items], "items": items} for k, items in by_kind.items()}
            hidden = _rows(c, "SELECT DISTINCT period_end, available_from FROM vi.statements WHERE ticker = ? AND available_from > ? ORDER BY 1", [ticker, t0])
