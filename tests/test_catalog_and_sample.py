...
def _video(con, vid, d, kind="single", ticker="NKE", conf=0.9):
    ...
    con.execute(
        "INSERT INTO vi.videos (video_id, title, published_at, kind, primary_ticker, year_bucket, date_source, catalogued_at) VALUES (?, ?, ?, ?, ?, ?, 'yt-dlp', ?)",
        [vid, vid, d, kind, ticker, yb, datetime.now(timezone.utc)],
    )
    con.execute(
        "INSERT INTO vi.title_labels VALUES (?, ?, '[]', ?, '', 'llm', 'test', ?)",
        [vid, kind, conf, datetime.now(timezone.utc)],
    )
        ...
        for m in range(12):
            for k in range(3):
                ...
                _video(
                    con,
                    f"v{n}",
                    date(d0.year + y + (1 if m >= 4 else 0), ((d0.month - 1 + m) % 12) + 1, 1 + k),
                    ticker=f"T{n % 17}",
                )
