...
def _seed(con):
    con.executemany(
        ...
        [
            ("NKE", date(2024, 6, 28), 95.0, 95.0),
            ("NKE", date(2024, 7, 1), 75.0, 75.0),
            ("NKE", date(2024, 7, 3), 74.0, 74.0),
            ("NKE", date(2025, 1, 2), 70.0, 70.0),
        ],
    )
    ...
    con.executemany(
        ...
        [
            ("NKE", date(2023, 5, 31), 51.2e9, date(2023, 8, 29)),
            ("NKE", date(2024, 5, 31), 51.4e9, date(2024, 8, 29)),
        ],
    )
...
def test_prices_as_of_excludes_future(con):
    _seed(con)
    rows = con.execute(
        "SELECT date FROM vi.prices_as_of('NKE', DATE '2024-07-02') ORDER BY date"
    ).fetchall()
...
def test_statements_as_of_uses_available_from_not_period_end(con):
    ...
    visible = con.execute(
        "SELECT period_end FROM vi.statements_as_of('NKE', DATE '2024-07-02')"
    ).fetchall()
    assert [r[0] for r in visible] == [date(2023, 5, 31)]
    later = con.execute(
        "SELECT count(*) FROM vi.statements_as_of('NKE', DATE '2024-09-01')"
    ).fetchone()[0]
    assert later == 2
...
def test_no_leak_property(con):
    ...
    for t0 in (date(2024, 1, 1), date(2024, 7, 2), date(2024, 12, 31)):
        assert (
            con.execute(
                "SELECT count(*) FROM vi.prices_as_of('NKE', ?) WHERE date > ?", [t0, t0]
            ).fetchone()[0]
            == 0
        )
        assert (
            con.execute(
                "SELECT count(*) FROM vi.statements_as_of('NKE', ?) WHERE available_from > ?",
                [t0, t0],
            ).fetchone()[0]
            == 0
        )
