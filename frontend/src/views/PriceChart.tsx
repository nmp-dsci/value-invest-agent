    ...
    <div>
      <svg className="chart" viewBox={`0 0 ${W} ${H}`} role="img" aria-label="price around the video date">
        ...
        {forward && Object.entries(forward).map(([m, f]) => { const v = validations?.find((x) => String(x.horizon_m) === m); return f ? (
          <g key={m}><circle cx={X(f.date)} cy={Y(f.close)} r="3.5" fill={v ? (v.verdict === 'correct' ? 'var(--good)' : v.verdict === 'wrong' ? 'var(--bad)' : 'var(--muted)') : 'var(--muted)'} /><text x={X(f.date)} y={Y(f.close) - 7} textAnchor="middle" fontSize="9" fontFamily="var(--mono)" fill={v ? (v.verdict === 'correct' ? 'var(--good)' : v.verdict === 'wrong' ? 'var(--bad)' : 'var(--muted)') : 'var(--muted)'}>+{m}m{v ? ` ${v.excess > 0 ? '+' : ''}${(v.excess * 100).toFixed(0)}pp` : ''}</text></g>
        ) : null; })}
