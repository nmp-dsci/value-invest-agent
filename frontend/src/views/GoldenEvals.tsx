    ...
    <div className="scroll"><div className="pagewrap">
      ...
      {sum && rows.length > 0 && (
        <>
          ...
          <div className="stats">
            ...
            <div className="stat"><div className="v">{st.iv_rule_n ? `${st.iv_rule_agrees} / ${st.iv_rule_n}` : '—'}</div><div className="l">his IV implies his call (rule D)</div><div className="d">IV vs price at T0, ±10 % = HOLD · split- and share-count-adjusted · {st.iv_total_basis ?? 0} whole-company IVs · {st.iv_unusable ?? 0} unusable (units)</div></div>
      ...
      {rows.length > 0 && (
        <div className="panel tablewrap" style={{ maxHeight: 'none' }}>
          ...
          <table><thead><tr><th>ticker · T0</th><th>title</th><th>split @ {h} m</th><th>position</th><th>stance</th><th className="num">expects</th><th className="num">IV → price · upside</th>{H.map((x) => <th key={x} className="num">+{x} m ex</th>)}<th>verdict @ {h} m</th><th className="num">repro.</th><th>critic</th><th>review</th></tr></thead>
            <tbody>{list.map((r) => { const v = r.validations?.[h]; return (
              <tr key={r.video_id} className="click" onClick={() => onOpen(r.video_id)}>
                ...
                <td className="num" title={r.checks?.iv_basis ? `stated ${r.iv_weighted_stated} · ${r.checks.iv_basis} · split ×${r.checks.split_factor}` : undefined}>{r.checks?.iv_comparable != null ? <>{r.checks.iv_comparable.toFixed(0)} → {r.price_at_t0?.toFixed(0) ?? '—'} <span style={{ color: r.checks.position_iv === 'BUY' ? 'var(--good)' : r.checks.position_iv === 'SELL' ? 'var(--bad)' : 'var(--muted)' }}>{r.checks.iv_upside_pct != null ? `${r.checks.iv_upside_pct > 0 ? '+' : ''}${r.checks.iv_upside_pct.toFixed(0)} %` : ''}</span>{r.checks.position_iv && r.checks.position_iv !== r.position && <span className="badge warn" style={{ marginLeft: 4 }}>D≠C</span>}</> : r.iv_weighted_stated != null ? <span className="badge">units?</span> : '—'}</td>
